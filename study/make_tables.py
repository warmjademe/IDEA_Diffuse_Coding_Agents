...
out=table('RQOneTable','tab:rq1',
    'D1 on normal Python reviews from three writers. There are 90 planned reviews from 30 tasks. '
...
for model in ('haiku','gpt'):
    for strategy in ('omission','downplay','bury'):
        ...
        rows.append([names[model],strategy.capitalize(),fraction(m['fpr']),fraction(m['fnr']),
                     fraction(m['joint_harmful_accepted']),f"${r['mean']*100:.2f}\\pm{r['sample_sd']*100:.2f}$"])
out+=table('RQTwoTable','tab:rq2',
    'Targeted Python attacks against D1. Each cell plans 90 reviews (30 tasks, three runs). '
...
for model in ('haiku','gpt'):
    for defense in ('D1','D4','D5'):
        ...
        rows.append([names[model],defense,fraction(a['joint_harmful_accepted'],False),
                     f"${100*r['mean']:.2f}\\pm{100*r['sample_sd']:.2f}$"])
out+=table('DefenseTable','tab:defense',
    'Descriptive joint evasion pooled across the three strategies. Each target/defense plans 270 attacks. '
    'D4 retains the initial D1 rubric in every trajectory; its separately sampled outcomes do not measure an effect of changed instructions. '
    'All 18 strategy-specific paired defense comparisons have BH-adjusted $q\\geq0.0738$.',
    'llll','Target & Defense & Joint evasion & Run mean$\\pm$SD (\\%)',rows)
rows=[]
for model in names:
    for defense in ('D1','D5'):
        a=p['summaries'][f'common/{model}/{defense}'];m=a['metrics']
        rows.append([names[model],defense,ci(m['auroc']),fraction(m['fpr']),fraction(m['fnr'])])
out+=table('FixedTable','tab:fixed',
    'Identical Python texts scored by every checker under D1 and D5. The six-condition common intersection contains 261 reviews '
    ...
    'lllll','Checker & Evidence & AUROC [CI] & FPR & FNR',rows,True)
rows=[]
for strategy in ('benign','omission','downplay','bury'):
    for defense in ('D1','D5'):
        a=s['groups'][f'java/{strategy}/haiku/{defense}'];m=a['metrics']
        rows.append(['Normal' if strategy=='benign' else strategy.capitalize(),defense,
...
for defense in ('D1','D5'):
    ...
    rows.append(['All',defense,str(a['usable']),fraction(a['fpr'],False),fraction(a['fnr'],False),number(a['auroc'])])
out+=table('JavaTable','tab:java',
    ...
    'Pooled D5--D1 FPR changes by $+1.87$ percentage points ($q=0.2344$), while FNR changes by $-15.09$ points ($q=0.046875$).',
    'llrlll','Instruction & Evidence & $n$ & FPR & FNR & AUROC',rows,True)
